...
export default {
  ...
  computed: {
    ...
    // Extrae la descripción de la colección
    getDescription() {
      return this.collection?.description?.value || this.collection?.description || ''
    }
  },

  // Carga la colección al montar el componente
  created() {
    this.fetchCollection()
  },

  methods: {
    // Pide el detalle de la colección por id
...
.records-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 1.5rem; }

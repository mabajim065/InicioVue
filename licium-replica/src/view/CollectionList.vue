      ...
      <div class="collections-grid">
        ...
        >
          ...
          <div class="col-body">
            ...
            <p v-if="getColDescription(col)" class="col-desc">
...
export default {
  ...
  computed: {
    totalPages() {
      // Calcula el número total de páginas basándose en los resultados
      return Math.ceil(this.totalCollections / this.limit)
    }
  },
  created() {
    this.fetchCollections() // Carga inicial
  },
  methods: {
    async fetchCollections() {
      ...
      try {
        ...
        // Adaptación flexible según cómo responda tu API
        this.collections = response.data.data || response.data.results || response.data || []
        this.totalCollections = response.data.total || response.data.count || this.collections.length
      } catch (err) {
...
<style scoped>
/* Estilos del listado de colecciones */
.page-header {
  text-align: center;
  margin-bottom: 2rem;
}
...
.collection-card:hover .col-image img {
